...
export class Paginator extends Component<PaginatorAttrs> {
    ...
    values = {
        ...
        current: {
            value: this.attrs && this.attrs.current ? this.attrs.current : 1
        },

        total: this.attrs && this.attrs.total ? this.attrs.total : 1,
        ...
        position: {
            ...
            view: () => {
                ...
                return (
                    this.attrs && this.attrs.positionView
                        ? this.attrs.positionView(ctx)
                        : new PositionView(ctx)
                ).render();
            }
        },
