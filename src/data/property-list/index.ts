 ...
 * an object.
 */
export class PropertyList<D, R extends Record<D>> extends Component<
    PropertyListAttrs<D, R>
> {
    ...
    values = {
        ...
        fields: <Field<D, R>[]>(
            (this.attrs && this.attrs.fields ? this.attrs.fields : [])
        ),

        data: {
            value: <R>(
                (this.attrs && this.attrs.data ? this.attrs.data : <any>{})
            ),

            get: (f: Field<D, R>) => {
                let mData = get(f.name, this.values.data.value);

                if (mData.isNothing()) return new NothingView({}).render();
                ...
                if (f.dataFragment) return f.dataFragment(ctx).render();
                else return new DataView(ctx).render();
            }
        }
    };
