...
export const doSetContent = (
    ...
    if (maybeRoot.isNothing()) return warnMissing(view, id);
    ...
    for (let i = 0; i < content.length; i++) n.appendChild(<Node>content[i]);
};
...
export const doRemoveContent = (view: View, id: string) => {
    ...
    if (maybeNode.isNothing()) return warnMissing(view, id);
