...
export type WMLElement = HTMLElement | Node | EventTarget | Widget 
 export function label(view) { return $$node('label',{html:{'for': this.attributes.read('ww:id'),'class': Styles.CONTROL_LABEL}},[this.attributes.read('ww:label')], view);}
export function message(view) { return $$node('span',{html:{'class': "help-block"},wml:{'id': "message"}},[this.attributes.read('ww:message','')], view);}
export class SelectView implements View{
  ...
  template: ()=>HTMLElement;
       ...
       static render(context) {
         ...
         return (new SelectView(context)).render();

       }

       register(id:string, w:WMLElement): SelectView{
  ...
  template: ()=>HTMLElement;
       ...
       constructor(context) {
          ...
          this.context = context;
          this.template = function(){
            return $$node('div',{html:{'class': [Styles.FORM_GROUP,this.attributes.read('ww:variant','')].join(',')}},[label.apply(this, [view].concat([])),$$if(this.attributes.read('ww:type','text') !== 'textarea', function if0(){return [$$node('input',{html:{'id': this.attributes.read('ww:id',''),'title': this.attributes.read('ww:title'),'name': this.attributes.read('ww:name',this.attributes.read('ww:id','')),'type': this.attributes.read('ww:type','text'),'placeholder': this.attributes.read('ww:placeholder'),'oninput': this.delegate.onInput.bind(this.delegate),'value': this.initialValue(),'disabled': this.attributes.read('ww:disabled',null),'readonly': this.attributes.read('ww:readonly',null),'class': Styles.INPUT},wml:{'id': "input"}},[], view)];}.bind(this),function else_clause3() { return [$$node('textarea',{html:{'id': this.attributes.read('ww:id',''),'title': this.attributes.read('ww:title'),'name': this.attributes.read('ww:name',this.attributes.read('ww:id','')),'class': Styles.TEXTAREA,'placeholder': this.attributes.read('ww:placeholder'),'oninput': this.delegate.onInput.bind(this.delegate),'disabled': this.attributes.read('ww:disabled',null),'readonly': this.attributes.read('ww:readonly',null),'rows': this.attributes.read('wat:rows')},wml:{'id': "input"}},[this.initialValue()], view)];}.bind(this)),message.apply(this, [view].concat([]))], view)
          }
       ...
       static render(context) {
         ...
         return (new InputView(context)).render();

       }

       register(id:string, w:WMLElement): InputView{
