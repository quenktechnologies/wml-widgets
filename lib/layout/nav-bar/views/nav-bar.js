...
class NavBarView {
    constructor(__context) {
        ...
        this.template = (__this) => {
            return __this.widget(new action_bar_1.ActionBar({ 'id': __context.values.id, 'className': __context.values.className }, [
                        ...
                        __this.widget(new nav_1.NavMenu({}, [
                            ...__forIn(__context.values.links, (conf, _$$i, _$$all) => ([
                                __this.widget(new item_1.Item({}, [
                                    __this.widget(new link_1.Link({ 'className': conf.className, 'name': conf.name, 'text': conf.title, 'href': conf.href, 'onClick': () => (conf.onClick) ? conf.onClick(conf.name) : undefined }, []), { 'className': conf.className, 'name': conf.name, 'text': conf.title, 'href': conf.href, 'onClick': () => (conf.onClick) ? conf.onClick(conf.name) : undefined })
                                ]), {})
                            ]), () => ([]))
                        ]), {})
                    ]))() :
