...
exports.rows = function (row, index, fields) { return function (___context) { return function (___view) { return ___wml.map(fields, function _map(field) {
    return ___wml.node('td', {
        ...
        wml: {}
    }, [(field.fragment) ? ___wml.domify(field.fragment(property_seek_1.get(field.name, row), field.name, row, field)(___view)) : ___wml.domify(property_seek_1.get(field.name, row))], ___view);
}, function otherwise() {
    return document.createDocumentFragment();
}); }; }; };
;
