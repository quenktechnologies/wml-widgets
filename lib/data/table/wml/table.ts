...
export class Main <C  ,R extends Record<C  >  >  implements __wml.View {
   ...
   constructor(__context: DataTable<C  ,R  > ) {
       ...
       this.template = (__this:__wml.Registry) => {
           ...
           return __this.widget(new TableLayout({wml : { 'id' : __context.values.wml.id  },'id': __context.values.id,'className': __context.values.className}, [

        __context.values.thead(),
__context.values.tbody()
     ]),<__wml.Attrs>{wml : { 'id' : __context.values.wml.id  },'id': __context.values.id,'className': __context.values.className});
