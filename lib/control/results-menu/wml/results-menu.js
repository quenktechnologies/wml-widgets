...
class Main {
    constructor(__context) {
        ...
        this.tree = __document.createElement('div');
        this.template = (__this) => {
            return __this.widget(new menu_1.Menu({ wml: { 'id': __context.values.wml.id }, ww: { 'className': __context.values.className, 'block': __context.values.block, 'hidden': __context.values.hidden } }, [
                ...__forIn(__context.values.results, (result, index, _$$all) => ([
                    __this.widget(new item_1.Item({ ww: { 'name': ('' + index) } }, [
                        __this.widget(new link_1.Link({ ww: { 'onClick': () => __context.values.item.click(index) } }, [
                            __this.registerView(__context.values.item.template(result, index)).render()
                        ]), { ww: { 'onClick': () => __context.values.item.click(index) } })
                    ]), { ww: { 'name': ('' + index) } })
                ]), () => ([
